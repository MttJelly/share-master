# Synclattice User Guide

[Back to README](../README.md) | [中文](USER_GUIDE.zh-CN.md)

Synclattice is a multi-model desktop client for Windows. It connects Codex, Claude Code, DeepSeek, Qwen, and OpenAI-compatible models in one workspace while keeping Synclattice's conversations in a separate data directory.

> [!IMPORTANT]
> Synclattice does not modify the application files of ChatGPT App, Codex, or Claude Code. Local configuration discovery and local conversation browsing access original data in read-only mode.

## 1. Installation and first launch

### Portable ZIP

1. Download the portable ZIP from [Releases](https://github.com/MttJelly/synclattice/releases).
2. Extract the entire archive to a normal folder. Do not run the app from inside the archive.
3. In the current stable package, double-click `Synclattice.exe`; the next package will use `Synclattice.exe`.

### MSI installer

1. Download and open the MSI package.
2. Complete the installation wizard.
3. Start Synclattice from the desktop shortcut or Start menu.
...
4. Complete sign-in on the official OpenAI ChatGPT page opened in your browser. Do not enter your ChatGPT email or password in Synclattice.

The local label only distinguishes accounts. Each separate account has its own authentication files while still using Synclattice's shared conversation history. An incomplete official login keeps the chat workspace locked.
...
API Keys are encrypted through Windows secure storage and written only to Synclattice's private data directory. Never place a key in a README, screenshot, Issue, or Git commit.
...
- Synclattice can send a Windows notification when background work completes. Check Windows notification permissions if none appears.
...
- Delete now (`立即删除`) in Removed skips the grace period and permanently deletes the corresponding Synclattice record.
...
Projects are reordered according to the latest activity of their conversations. Deleting a Project removes its Synclattice grouping and assignments but does not delete its linked local directory.
...
To keep tasks running after closing the window, enable Keep running in tray after closing (`关闭窗口后留在托盘`) in App settings and leave Synclattice running. Tasks cannot execute while the computer is off or Synclattice is fully exited.
...
Scanning does not modify source files. Only confirmed selections are written to Synclattice. Secrets are stored in Synclattice's encrypted secure storage and are not displayed in scan results.
...
4. To continue it in Synclattice, explicitly choose the copy/import action.

Browsing alone does not import, delete, or rewrite original records. A copied conversation belongs to Synclattice, and later changes are not written back to the original client.

## 12. Skills, Prompts, and MCP

Open Extension center (`扩展中心`) from Select connection:

- Skills: install from a folder, ZIP, or GitHub source into Synclattice's private Skill directory. You can refresh, enable, disable, or remove installed Skills.
...
Synclattice's usage logs do not store message bodies or credentials.

## 14. Import, export, backup, and sync

### Configuration import and export

Import (`导入`) and Export (`导出`) transfer Synclattice configuration. An import preview appears before anything is written. API Key, Token, password, and credential fields in links are rejected.
...
Sync covers Synclattice configuration, not API Keys, MCP secrets, or conversation bodies. Credentials must be configured separately on each device.

After importing on a new computer, provider URLs and the last discovered model names are retained, but API Keys are intentionally omitted. Selecting a relay without a stored key opens its editor directly; enter the key, choose Test connection, and Synclattice will refresh `/models` (or `/v1/models`). Once models are loaded, the composer exposes conservative low, medium, and high reasoning choices for compatible models. The provider decides whether a selected level is honored; Synclattice sends it as `reasoning_effort`.
...
Open Conversation location (`记录位置`) from Select connection to view or change Synclattice's conversation directory. Changing it does not move or delete records in the previous directory. Verify source and destination before running directory synchronization.

Uninstalling Synclattice does not automatically delete `%APPDATA%\Synclattice`. Back up any conversations you need before removing that directory manually.
...
6. Fully exit and restart Synclattice. If tray mode is enabled, exit from the tray menu as well.
...
Confirm that the task is enabled, its time and local timezone are correct, its selected connection is available, and Synclattice was not fully exited at the due time.
...
- Synclattice's private conversations, account configuration, and cache are independent of the original clients.
- Local configuration discovery and local conversation browsing do not modify source files.
- Synclattice does not delete or overwrite original conversations from ChatGPT App, Codex, or Claude Code.
...
For a reproducible problem, include the Synclattice version, Windows version, connection type, reproduction steps, and a redacted error in a GitHub Issue. Never upload API Keys, Tokens, private conversations, or full logs containing personal paths.
