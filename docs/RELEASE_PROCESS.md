...
本流程适用于向 GitHub 推送 Synclattice 的产品代码更新。普通文档纠错可合并处理；只要推送包含功能、修复或用户体验变化，就必须完成以下步骤。
...
6. 创建发布包时，同时上传带版本号文件和 `Synclattice-portable-win-x64.zip`、`Synclattice-setup-win-x64.msi` 稳定别名，并核对 `release-manifest.json` 的 SHA-256。
