# Give Share Master a semantic, conversation-first color system
...
- Owner: the final Share Master theme layer in `src/renderer/styles.css`, with existing structure and labels in `src/renderer/index.html`
...
- Exclude: provider execution, model reasoning, conversation persistence, WebDAV payload design, original client data, packaging, release metadata, publishing, and the user's active Share Master window.
