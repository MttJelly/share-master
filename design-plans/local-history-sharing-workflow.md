# Make local history a clear Share Master sharing workflow
...
- Problem: the surface describes itself as read-only and ends at preview, while the user guide and product positioning say a local Codex or Claude conversation can be copied into Share Master for cross-model continuation. There is no visible copy command or import state in the preview.
...
Turn the existing three-pane browser into a source-to-Share-Master workflow without weakening its read-only boundary. Keep source navigation and preview unchanged, then place one compact primary command in the selected preview heading: “复制到 Share Master”. Pair it with an inline status region that explains the private-copy result. The copy command must never imply that the source file is moved or synchronized, and duplicate imports must resolve to the existing Share Master copy.
...
1. `src/renderer/app.js`
   - Change: add the selected-conversation copy command and inline progress/result state to the preview heading; after a successful copy, refresh the Share Master conversation list and open the copied thread when a provider is connected.
...
- Product: select a fixture Codex/Claude conversation, copy it into an isolated Share Master profile, verify its messages and reasoning summary, repeat the action, then continue through another provider branch.
